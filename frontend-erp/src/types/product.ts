...
// A size variant with its own independent price tiers (for multi-size products)
export interface SizeVariantTier {
...
export interface SizeVariant {
    id?: string;
    size: string;      // e.g. "20x30"
    label: string;     // display label e.g. "20×30 cm"
    tiers: SizeVariantTier[];
    bom_items?: BomItem[];
}

// Pricing mode: 'flat' = single global price tiers, 'multi-size' = per-size price tiers
export type PricingMode = 'flat' | 'multi-size';
...
export interface ProductFormData {
    ...
    // Pricing
    pricing_mode: PricingMode;   // 'flat' or 'multi-size'
    base_price: number;
    price_tiers: PriceTier[];       // used when pricing_mode === 'flat'
    size_variants: SizeVariant[];   // used when pricing_mode === 'multi-size'
